...
static uint8_t data_global[256] = {0};
...
static void i2c_slave_handler(i2c_inst_t *i2c,i2c_slave_event_t event) {
    switch (event) {
        case I2C_SLAVE_RECEIVE:{ // receive from master
            ...
            data_global[reg_address++] = data;
            break;
        }
        case I2C_SLAVE_REQUEST:{ // send to master
            if(reg_address < strlen(data_to_send)){
                i2c_write_byte_raw(i2c, data_to_send[reg_address]);
                reg_address++;
            } else {
                reg_address = 0;
            }
